...
import { h } from '../agc-oat-harvest-loss.core.js';
...
const mapEnterKey = (form) => {
    return (e) => {
        ...
        if (self.classList.contains('agc-wizard__actions-next')) {
            return;
        }
        const enterKey = () => {
...
class AgcOatHarvestLoss {
    ...
    render() {
        return (h("div", null,
            h("form", { onSubmit: e => e.preventDefault(), ref: c => (this.form = c), "data-wizard": "agc-oat-harvest-loss", "data-wizard-mode": this.mode, class: "agc-wizard" },
                h("slot", null),
                h("section", { "data-wizard-section": "0" },
                    h("div", { class: "agc-wizard__field" },
                        h("label", { "data-i18n": "fields.kernels-counted" }, "Kernels Counted"),
                        h("input", { name: "kernelsCounted", type: "number", required: true, min: "0", step: "1" }),
                        h("p", { class: "agc-wizard__validation-message", "data-i18n": "validation.kernels-counted.required", "data-validates": "kernelsCounted" }, "Please enter a whole number of zero or greater."),
                        h("p", { "data-i18n": `hints.kernels-counted` }, "\u2BA4 Enter the number of oat kernels counted on the ground within the specified area.")),
                ...
                h("section", { "data-wizard-section": "1" },
                    h("div", { class: "agc-wizard__field" },
                        ...
                        h("p", { "data-i18n": `hints.loss-area` }, "\u2BA4 Enter the area in square feet that was used to count oat kernels on the ground.")),
    ...
    showResults() {
        let kernelsCounted = parseInt(this.form.querySelector('[name="kernelsCounted"')
            .value);
        let lossArea = parseInt(this.form.querySelector('[name="lossArea"').value);
        let yieldLoss = round(kernelsCounted / lossArea / 10, 2);
        let results = {
            socket: this.socket,
            tract: this.tract,
            units: this.units,
    ...
    static get is() { return "agc-oat-harvest-loss"; }
